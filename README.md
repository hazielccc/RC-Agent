...
# 5. 启动 FastAPI 主服务（新开一个 PowerShell 窗口）
# 注意：日志会自动输出到 logs\app_YYYY-MM-DD.log
python -m uvicorn app.main:app --host 0.0.0.0 --port 9900

# 6. 上传文档到 RAG-Anything 知识库（新开一个 PowerShell 窗口）
# 等待服务启动完成后执行
timeout /t 5
python -c "import requests, pathlib, time; exts={'.pdf','.jpg','.jpeg','.png','.bmp','.tiff','.tif','.gif','.webp','.doc','.docx','.ppt','.pptx','.xls','.xlsx','.txt','.md'}; [requests.post('http://localhost:9900/api/upload', files={'file': open(p, 'rb')}) or time.sleep(1) for d in ['aiops-docs','my_knowledge_document'] for p in pathlib.Path(d).rglob('*') if p.is_file() and p.suffix.lower() in exts]"
```
...
- **局域网访问**: 当前主机执行 `make network-url` 获取地址，其他主机访问 `http://<当前主机IP>:9900/static/index.html`
- **公网访问**: 安装 `cloudflared` 后执行 `make public-url`，终端会输出可在不同网络访问的 HTTPS 地址。

公网访问建议配置访问令牌，避免服务裸露在互联网上。在 `.env` 中添加：

```bash
PUBLIC_ACCESS_TOKEN=请替换为一段足够长的随机字符串
```

配置后公网地址使用：

```text
https://<cloudflared生成的域名>/static/index.html?access_token=<PUBLIC_ACCESS_TOKEN>
```
...
| 本地文件索引 | POST | `/api/index_file` | 索引项目内本地文档 |
| 目录索引 | POST | `/api/index_directory` | 递归索引目录文档 |
...
│   └── README.md                           # MCP 服务说明
├── aiops-docs/                             # 内置运维知识库
├── my_knowledge_document/                  # 用户本地知识库文件
...
# RAG 配置
RAG_TOP_K=3
LOCAL_KNOWLEDGE_DIR=./my_knowledge_document
RAG_ANYTHING_WORKING_DIR=./rag_anything_storage
RAG_ANYTHING_PARSER_OUTPUT_DIR=./rag_anything_output
RAG_ANYTHING_PARSER=mineru
RAG_ANYTHING_PARSE_METHOD=auto
RAG_ANYTHING_MINERU_BACKEND=pipeline
RAG_ANYTHING_IMAGE_OCR_ENABLED=True
RAG_ANYTHING_IMAGE_OCR_MODEL=qwen-vl-ocr-latest
RAG_ANYTHING_IMAGE_OCR_TIMEOUT=120
RAG_ANYTHING_IMAGE_OCR_MAX_TOKENS=4096
RAG_ANYTHING_QUERY_MODE=hybrid
CHUNK_MAX_SIZE=800
CHUNK_OVERLAP=100
```

本地知识库文件放入 `my_knowledge_document/` 后，执行 `make init` 或 `make upload` 会自动写入智能体对话使用的 RAG-Anything 知识库。当前跟随 RAG-Anything 支持的文件类型：PDF、图片（jpg/jpeg/png/bmp/tiff/tif/gif/webp）、Office（doc/docx/ppt/pptx/xls/xlsx）、TXT、Markdown。为避免大量 Office/PDF 文档经 OCR 解析触发 macOS MPS 内存溢出，docx/pptx/xlsx 默认使用本地文本解析，doc/ppt/xls 默认优先使用 macOS `textutil` 或 LibreOffice 转新版格式后抽文本，PDF 默认优先抽取文本层；图片 OCR 使用阿里云百炼 Qwen 视觉 OCR 模型，默认 `qwen-vl-ocr-latest`。
...
```bash
# 项目管理
make init              # 一键初始化（仅启动服务，不更新 RAG）
make start             # 启动所有服务
make stop              # 停止所有服务
make restart           # 重启所有服务
make network-url       # 显示局域网访问地址
make public-url        # 生成公网访问地址（需要 cloudflared）
make stop-public       # 停止公网隧道

# 依赖管理
make install-dev       # 安装开发依赖
make sync              # 同步依赖

# Docker 管理（可选，仅历史 Milvus 环境需要）
...
### RAG-Anything 初始化失败
```bash
# 首次初始化会下载 tiktoken 编码缓存，请确认网络可访问
.venv/bin/python -c "import asyncio; from app.services.rag_anything_service import rag_anything_service; asyncio.run(rag_anything_service.ensure_ready())"

# 如需重建本地 RAG-Anything 知识库，可删除本地存储后重新上传文档
rm -rf rag_anything_storage rag_anything_output uploads
make start
make upload
```
