...
保留原有 vector_index_service 名称，兼容现有 API 调用。
"""
...
class VectorIndexService:
    """RAG-Anything 索引服务 - 负责读取文件并写入 RAG-Anything."""

    def __init__(self):
        """初始化向量索引服务"""
        self.upload_path = "./uploads"
        logger.info("RAG-Anything 索引服务初始化完成")

    async def aindex_directory(self, directory_path: Optional[str] = None) -> IndexingResult:
        """
        索引指定目录下的所有文件
        ...
        try:
            ...
            # 获取所有 RAG-Anything 支持的文件，递归扫描本地知识库目录。
            files = sorted(
                file_path
                for file_path in dir_path.rglob("*")
                if file_path.is_file()
                and file_path.suffix.lower() in RAG_ANYTHING_SUPPORTED_EXTENSIONS
            )
            ...
            for file_path in files:
                try:
                    await self.aindex_single_file(str(file_path))
    ...
    def index_directory(self, directory_path: Optional[str] = None) -> IndexingResult:
        """同步索引目录，供脚本场景兼容使用。"""
        return asyncio.run(self.aindex_directory(directory_path))

    def index_single_file(self, file_path: str):
        """同步索引单个文件，供脚本场景兼容使用。"""
        return asyncio.run(self.aindex_single_file(file_path))

    async def aindex_single_file(self, file_path: str) -> str:
        """
        索引单个文件到 RAG-Anything
        ...
        try:
            doc_id = await rag_anything_service.index_file(str(path))
            logger.info(f"文件索引完成: {file_path}, RAG-Anything doc_id={doc_id}")
            return doc_id
