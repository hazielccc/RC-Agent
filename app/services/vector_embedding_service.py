...
def _build_vector_embedding_service() -> Embeddings:
    provider = (config.embedding_provider or "dashscope").strip().lower()
    if provider in {"bge-m3-mlx", "mlx", "local"}:
        from app.services.local_embedding_service import local_embedding_service

        logger.info(
            f"使用本地 Embedding 服务: {config.local_embedding_model_name}, "
            f"path={config.local_embedding_model_path}, 维度={config.local_embedding_dim}"
        )
    ...
    if provider in {"dashscope", "openai"}:
        return DashScopeEmbeddings(
            ...
            dimensions=config.local_embedding_dim,
        )
...
# 全局单例
vector_embedding_service = _build_vector_embedding_service()
